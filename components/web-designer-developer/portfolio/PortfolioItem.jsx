...
export default function PortfolioItem({ item, onClick }) {
  ...
  return (
    <div className="group">
      <button
        onClick={onClick}
        className="block w-full h-full cursor-pointer rounded-2xl overflow-hidden"
          ...
          {item.type === "image" && (
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-colors duration-150 ease-in-out flex items-end justify-center pb-20 px-2">
              <div className="opacity-0 group-hover:opacity-100 transition-all duration-150 space-y-2">
                <div className="relative max-w-max py-2">
                  <h3 className="text-2xl font-rajdhani font-bold text-text-primary leading-none max-w-[30ch] text-left">
                    {item.title}
                  </h3>
                </div>
                <p className="font-normal text-text-primary max-w-[30ch] text-left">
                  {item.description}
                </p>
              </div>
            </div>
